...
describe('PizzaSort.vue', () => {
  it('Emits event with params when clicked buttons', async () => {
    ...
    await wrapper
      .findAll('[data-test=dough-btn]')
      .at(0)
      .trigger('click');
    expect(wrapper.emitted('dough-selected')[0][0]).toEqual(DOUGH_OPTIONS[0]);
    await wrapper
      .findAll('[data-test=size-btn]')
      .at(1)
      .trigger('click');
    expect(wrapper.emitted('size-selected')[0][0]).toEqual(SIZE_OPTIONS[1]);
  });
});
